...
struct BootChainStepView: View {
    ...
    var body: some View {
        WizardStepContainerView(
            title: "Boot Chain",
            bodyText: "Deploy the boot chain (m1n1 bootloader + Linux kernel + initramfs) to the Cidre System volume. This is required for the Mac to recognize Cidre as a bootable OS."
        ) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if bootPolicyVM.allComponentsReady {
                    ...
                    Text("Run the operation below to stage the boot chain onto the Cidre volume.")
                        .font(.callout)
