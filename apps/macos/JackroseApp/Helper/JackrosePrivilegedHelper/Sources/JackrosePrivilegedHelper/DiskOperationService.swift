...
enum DiskOperationService {
    ...
    static func execute(_ request: HelperProtocol) -> Result {
        ...
        if request.operation == "partition-create" || request.operation == "partition-delete" || request.operation == "jackrose-uninstall" {
        ...
        if request.operation == "jackrose-uninstall" {
            return executeJackroseUninstall(request, stubTarget: target)
        }
        ...
        case "partition-delete":
            ...
            command = ["/usr/sbin/diskutil", "eraseVolume", "free", "JackroseFreeSpace", target]
        default:
    ...
    private static func executeJackroseUninstall(_ request: HelperProtocol, stubTarget: String) -> Result {
        guard request.arguments.count == 4 else { return rejected("Jackrose uninstall requires EFI, Linux, Recovery, and startup APFS targets.") }
        ...
        guard stub["Content"] as? String == "Apple_APFS",
              ...
              containsJackroseVolume(containerInfo) else {
            return rejected("The first target is not a Jackrose APFS stub container.")
        }
        let efiName = (efi["VolumeName"] as? String ?? "").uppercased()
        guard efi["Content"] as? String == "EFI", efiName.contains("JACKROSE") else {
            return rejected("The EFI target is not labeled for Jackrose.")
        }
        ...
        guard partitionsAreContiguous(dictionaries) else {
            return rejected("Jackrose partitions are not contiguous with the macOS container.")
        }
        ...
        if request.dryRun {
            return Result(status: "pass", summary: "Validated complete Jackrose uninstall preview.", exitCode: 0, command: preview, stdout: preview.joined(separator: "\n"), stderr: "")
        }
        var output = ""
        for command in commands {
            ...
            if result.status != 0 {
                return Result(status: "fail", summary: "Jackrose uninstall stopped because diskutil failed.", exitCode: result.status, command: preview, stdout: output, stderr: result.error)
            }
        }
        return Result(status: "pass", summary: "Old Jackrose partitions were removed and macOS space was restored.", exitCode: 0, command: preview, stdout: output, stderr: "")
    }

    private static func containsJackroseVolume(_ value: Any) -> Bool {
        if let dictionary = value as? [String: Any] {
            if let name = dictionary["Name"] as? String, name.lowercased().contains("jackrose") { return true }
            return dictionary.values.contains(where: containsJackroseVolume)
        }
        if let values = value as? [Any] { return values.contains(where: containsJackroseVolume) }
        return false
    }
