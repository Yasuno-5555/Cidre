        ...
        if isMockMode {
            ...
            if command.contains("install-preflight") {
                ...
                return CommandResult(
                    ...
                    artifacts: nil,
                    nextActions: [
                        NextAction(label: "Review dry-run plan", command: "scripts/jackrose-uninstall-dry-run --target <target>")
                    ]
                )
            }
        }
