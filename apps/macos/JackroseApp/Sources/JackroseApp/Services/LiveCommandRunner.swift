...
class LiveCommandRunner {
    ...
    private func resolvedEnvironment(for workingDir: String) -> [String: String] {
        ...
        let testModePath = URL(fileURLWithPath: workingDir)
            .appendingPathComponent(".local/state/jackrose/mutation/current/test-mode.json")
        ...
        environment["JACKROSE_MUTATION_TEST_MODE"] = "1"
        return environment
    }
    
    private func mockResult(for command: String, arguments: [String], workingDir: String) -> CommandResult {
        let fixturesDir = URL(fileURLWithPath: workingDir).appendingPathComponent("apps/macos/JackroseApp/Fixtures")
