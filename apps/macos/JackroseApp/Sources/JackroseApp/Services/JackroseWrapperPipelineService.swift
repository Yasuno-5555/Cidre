...
public class JackroseWrapperPipelineService {
    private let runner: JackroseWrapperScriptRunner
    private let decoder: JSONDecoder
    private let repoRoot: URL
    
    public init(runner: JackroseWrapperScriptRunner = JackroseWrapperScriptRunner(), repoRoot: URL = URL(fileURLWithPath: "/Users/yasuno/Projects/Jackrose")) {
        self.runner = runner
    ...
    public func selectImage() async throws -> SelectedImage {
        let res = try await runner.runScript(name: "jackrose-wrapper-select-image", arguments: [
            "--metadata", repoRoot.appendingPathComponent("installer/fixtures/jackrose-seed-v0.local.json").path,
    ...
    public func fetchArtifact() async throws -> VerifiedArtifact {
        let res = try await runner.runScript(name: "jackrose-wrapper-fetch-artifact", arguments: [
    ...
    public func inspectArtifact() async throws -> ArtifactStructure {
        let res = try await runner.runScript(name: "jackrose-wrapper-inspect-artifact", arguments: [
    ...
    public func extractSandbox() async throws -> ExtractedRootfs {
        ...
        let res = try await runner.runScript(name: "jackrose-wrapper-extract-sandbox", arguments: args)
    ...
    public func validateRootfs() async throws -> RootfsValidation {
        let res = try await runner.runScript(name: "validate-jackrose-extracted-rootfs-content", arguments: [
    ...
    public func generateInstallPlan() async throws -> JackroseInstallPlan {
        let res = try await runner.runScript(name: "generate-jackrose-install-plan", arguments: [
        ...
        return try loadJSON(JackroseInstallPlan.self, from: "install-plan.json")
    }
    ...
    public func discoverTargets(fixture: String? = nil) async throws -> TargetCandidates {
        ...
        let res = try await runner.runScript(name: "jackrose-wrapper-discover-targets", arguments: args)
    ...
    public func selectTarget(targetID: String, confirm: String) async throws -> SelectedTarget {
        let res = try await runner.runScript(name: "jackrose-wrapper-select-target", arguments: [
    ...
    public func bindFinalContract() async throws -> FinalInstallContract {
        let res = try await runner.runScript(name: "bind-jackrose-final-install-contract", arguments: [
            "--rootfs-validation", getWrapperPath("rootfs-validation.json").path,
            "--install-plan", getWrapperPath("install-plan.json").path,
    ...
    public func generateDryRunPlan() async throws -> DryRunStagingPlan {
        let res = try await runner.runScript(name: "generate-jackrose-dryrun-staging-plan", arguments: [
    ...
    public func stageTargetWithApply(confirm: String, fixtureMode: Bool = false) async throws -> StagingResult {
        ...
        if fixtureMode {
            args.append(contentsOf: ["--fixture-mode", "--mount-root", "/tmp/jackrose-target-mount"])
        }
        let res = try await runner.runScript(name: "jackrose-wrapper-stage-target", arguments: args)
    ...
    public func validateStagedTarget(fixtureMode: Bool = false) async throws -> StagedTargetValidation {
        ...
        let res = try await runner.runScript(name: "validate-jackrose-staged-target", arguments: args)
    ...
    public func generateFirstBootHandoff() async throws -> FirstBootHandoff {
        let res = try await runner.runScript(name: "generate-jackrose-firstboot-handoff", arguments: [
    ...
    public func freezeInstallerMVP() async throws -> InstallerMVPFreeze {
        let res = try await runner.runScript(name: "freeze-jackrose-installer-mvp", arguments: [
            "--staging-result", getWrapperPath("staging-result.json").path,
            "--staged-validation", getWrapperPath("staged-target-validation.json").path,
