...
class RepositoryPathStore {
    ...
    private let pathKey = "JackroseRepositoryPath"
    ...
    func loadPath() -> String {
        if let savedPath = UserDefaults.standard.string(forKey: pathKey), validatePath(savedPath).valid {
            ...
            let bundledMarker = URL(fileURLWithPath: expandedPath).appendingPathComponent(".jackrose-backend").path
    ...
    func validatePath(_ path: String) -> RepositoryStatus {
        ...
        let hasMacOSApp = fm.fileExists(atPath: pathUrl.appendingPathComponent("apps/macos/JackroseApp").path)
        let isBundledBackend = fm.fileExists(atPath: pathUrl.appendingPathComponent(".jackrose-backend").path)
        ...
        if !exists {
            ...
            if !hasMacOSApp && !isBundledBackend { errors.append("Missing Jackrose app backend marker") }
    ...
    private func locateDefaultPath() -> String {
        ...
        let candidates = [
            homeDir.appendingPathComponent("Projects/Jackrose").path,
            homeDir.appendingPathComponent("Jackrose").path
        ]
    ...
    private func installBundledBackend() -> String? {
        let fm = FileManager.default
        guard let source = Bundle.main.resourceURL?.appendingPathComponent("JackroseBackend"),
              fm.fileExists(atPath: source.appendingPathComponent(".jackrose-backend").path) else { return nil }
        let support = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        let destination = support.appendingPathComponent("Jackrose/Backend", isDirectory: true)
        do {
