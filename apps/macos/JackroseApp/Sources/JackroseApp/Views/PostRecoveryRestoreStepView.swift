...
struct PostRecoveryRestoreStepView: View {
    ...
    var body: some View {
        WizardStepContainerView(
            ...
            bodyText: "Confirm that Reduced Security is enabled for Jackrose. This step verifies the security configuration you set in Startup Security Utility."
        ) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        if case .reducedSecurity = bootPolicyVM.securityMode {
                            HStack(spacing: 8) {
                                ...
                                Text("Jackrose is ready to boot from Startup Options").font(.callout)
                            }
                        }
                    }
                    .padding(.vertical, 4)
                    ...
                    GroupBox {
                        VStack(alignment: .leading, spacing: 8) {
                            HStack(spacing: 8) {
                                ...
                                Text("Reduced Security is not yet enabled for Jackrose.")
                                    .font(.callout)
                            }
                            Text("Restart into macOS Recovery (hold power button at boot), open Startup Security Utility, select Jackrose, and set Security Policy to Reduced Security. Then return here and run this verification again.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        .padding(8)
