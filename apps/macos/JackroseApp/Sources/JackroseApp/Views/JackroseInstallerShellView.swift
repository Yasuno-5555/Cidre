...
public struct JackroseInstallerShellView: View {
    @StateObject private var viewModel = JackroseInstallerShellViewModel()
    ...
    public init(fixtureMode: Bool = false, fakeMount: Bool = false) {
        ...
        let service = JackroseWrapperPipelineService()
        let vm = JackroseInstallerShellViewModel(service: service)
    ...
    public var body: some View {
        VStack {
            ...
            case .artifactVerification:
                JackroseInstallerProgressView(statusText: getStatusText())
            case .diskPlanning:
                diskPlanningScreen
            case .finalReview:
                if let contract = viewModel.finalContract, let plan = viewModel.dryRunPlan {
                    JackroseInstallerFinalReviewView(contract: contract, plan: plan) {
                        viewModel.confirmReview()
                    }
                } else {
                    JackroseInstallerProgressView(statusText: "Analyzing staging contracts...")
                }
            case .applyConfirmation:
                JackroseInstallerApplyConfirmationView { confirmation in
            ...
            case .firstBootHandoff:
                if let handoff = viewModel.firstBootHandoff {
                    JackroseFirstBootHandoffView(handoff: handoff) {
                        viewModel.currentStep = .complete
                    }
                } else {
                    JackroseInstallerProgressView(statusText: "Aggregating freeze metadata...")
                }
            case .complete:
                completionScreen
            case .failed(let msg):
    ...
    private var completionScreen: some View {
        VStack(spacing: 20) {
            ...
            Text("Jackrose Installation Done")
                .font(.title)
                .bold()
