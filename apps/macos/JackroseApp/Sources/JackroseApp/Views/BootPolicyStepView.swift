...
struct BootPolicyStepView: View {
    ...
    var body: some View {
        WizardStepContainerView(
            title: "Boot Policy",
            bodyText: "Register Jackrose in the Mac's Startup Options. This requires your macOS owner password to authorize the boot policy creation."
        ) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                // 1TR Auto-Run setup guidance (Asahi-style: Jackrose is default boot → auto-boots into 1TR)
                if bootPolicyVM.oneTrReady && bootPolicyVM.step2Ready {
                    GroupBox {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            Text("Jackrose is set as the default boot. On restart, your Mac will automatically boot into the Jackrose setup environment (1TR — One True Recovery) using an Apple-signed kernelcache. Setup completes in 1–2 minutes, then your Mac restarts back to macOS. No manual steps needed.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                StepView(number: 4, icon: "hand.tap", title: "Hold power button to boot Jackrose", detail: "From now on, Jackrose appears in Startup Options → Linux boots!")
                            }

                            Divider()

                            Text("This is the same approach used by the Asahi Linux installer (asahi-alarm reference). Jackrose is default boot for ONE reboot only — during that boot, bputil sets Reduced Security and kmutil installs m1n1 (entry-point 2048, matching Asahi's boot.bin layout). macOS is restored as default automatically after setup.")
                                .font(.caption2)
                                .foregroundColor(.secondary)
                ...
                // 1TR pending but step2 not fully staged (Jackrose IS default boot, user needs to run step2 manually)
                if bootPolicyVM.oneTrReady && !bootPolicyVM.step2Ready {
                    GroupBox {
                        VStack(alignment: .leading, spacing: 12) {
                            HStack(spacing: 8) {
                            ...
                            Text("Jackrose is set as default boot. On restart, your Mac will boot into the Jackrose recovery environment. Open Terminal (Utilities → Terminal) and run the step2 script to complete setup.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                            ...
                            Text("After step2 completes, your Mac restarts to macOS and Jackrose appears in Startup Options.")
                                .font(.caption2)
                                .foregroundColor(.secondary)
                        }
                        .padding(8)
                ...
                if !bootPolicyVM.oneTrReady && !bootPolicyVM.step2Ready {
                    GroupBox {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            Text("Your Mac uses Full Security, which requires Apple-signed bootloaders. To allow Jackrose's unsigned bootloader (m1n1), you must enable Reduced Security from macOS Recovery.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                ...
                if case .reducedSecurity = bootPolicyVM.securityMode {
                    GroupBox {
                        HStack(spacing: 8) {
                            Image(systemName: "checkmark.shield.fill")
                                .foregroundColor(.green)
                            Text("Reduced Security is configured. Jackrose is ready to boot.")
                                .font(.callout)
                        }
                        .padding(8)
                    }
