...
@MainActor
public class JackroseInstallerShellViewModel: ObservableObject {
    ...
    @Published public var installPlan: JackroseInstallPlan? = nil
    ...
    private let service: JackroseWrapperPipelineService
    
    public init(service: JackroseWrapperPipelineService = JackroseWrapperPipelineService()) {
    ...
    public func discoverTargets() async {
        ...
        do {
            let fixturePath = fixtureMode ? "/Users/yasuno/Projects/Jackrose/installer/fixtures/target-discovery/lsblk.sample.json" : nil
    ...
    public func selectCandidateTarget(_ candidate: TargetCandidate) async {
        ...
        do {
            selectedTarget = try await service.selectTarget(targetID: candidate.target_id, confirm: "SELECT JACKROSE TARGET ONLY - NO INSTALL")
