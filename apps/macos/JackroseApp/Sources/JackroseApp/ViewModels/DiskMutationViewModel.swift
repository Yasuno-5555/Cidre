...
enum PartitionMode: String, CaseIterable {
    ...
    var description: String {
        switch self {
        case .max:    return "Largest possible Jackrose partition (macOS gets its recommended minimum)"
        case .auto:   return "Balanced split — ~60% of available space for Jackrose"
        case .min:    return "Smallest useful Jackrose partition (20 GB)"
...
final class DiskMutationViewModel: ObservableObject {
    ...
    @Published var volumeName = "Jackrose"
    ...
    var canPreview: Bool {
        let isSafeVolume = installTarget?.classification == "jackrose-volume"
    ...
    var hasPreviewResult: Bool {
        execution?.command == "scripts/jackrose-app-helper-command"
    }

    var hasMutationExecutionResult: Bool {
        execution?.command == "scripts/jackrose-app-mutation-execute-test"
    }
    ...
    func enableMutationTestMode(repositoryPath: String) {
        runGuidedAction(
            ...
            command: "scripts/jackrose-app-mutation-test-mode",
    ...
    func enableInstallerTestOverride(repositoryPath: String) {
        runGuidedAction(
            ...
            command: "scripts/jackrose-app-installer-killswitch",
    ...
    func captureSnapshot(label: String, repositoryPath: String) {
        runGuidedAction(
            ...
            command: "scripts/jackrose-app-disk-snapshot",
    ...
    func generateRollbackReport(repositoryPath: String) {
        runGuidedAction(
            ...
            command: "scripts/jackrose-app-gate-report",
            arguments: ["--json"]
        )
    }
    ...
    func refreshBootSafety(repositoryPath: String) {
        runGuidedAction(
            ...
            command: "scripts/jackrose-app-boot-safety-gate",
            arguments: ["--json"]
        )
    }
    ...
    func refreshKillSwitch(repositoryPath: String) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let execution = LiveCommandRunner.shared.run(
                "scripts/jackrose-app-installer-killswitch",
    ...
    func resolveCreatePlanBlockers(repositoryPath: String) {
        ...
        if !isSafeVolume && mutationTestMode?.enabled != true {
            ...
            guidedActionExecution = LiveCommandRunner.shared.run(
                "scripts/jackrose-app-mutation-test-mode",
                arguments: ["--enable", "--phrase", "I understand this can destroy the selected disposable target.", "--json"],
                repositoryPath: repositoryPath,
                isMockMode: false
            )
    ...
    func createPlan(repositoryPath: String) {
        run(repositoryPath: repositoryPath, command: "scripts/jackrose-app-disk-plan", arguments: planArguments)
    ...
    func preview(repositoryPath: String) {
        ...
        run(repositoryPath: repositoryPath, command: "scripts/jackrose-app-helper-command", arguments: helperArguments(planID: planID, dryRun: true))
        refreshSafetyStatus(repositoryPath: repositoryPath)
    }
    ...
    func execute(repositoryPath: String) {
        ...
        run(repositoryPath: repositoryPath, command: "scripts/jackrose-app-mutation-execute-test", arguments: executionArguments(planID: planID))
        refreshSafetyStatus(repositoryPath: repositoryPath)
    }
