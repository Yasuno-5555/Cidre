...
/// Boot security mode for the Jackrose Volume Group
...
final class BootPolicyViewModel: ObservableObject {
    ...
    /// Whether Jackrose is set as default boot for 1TR auto-setup
    @Published var oneTrReady: Bool = false
    /// Whether the automated step2 script is staged on the Jackrose volume
    ...
    func updateFromResult(_ result: [String: Any]?) {
        ...
        if let status = result["status"] as? String, status == "blocked",
           let command = result["command"] as? String, command == "jackrose-app-m1n1-build" {
