...
This document outlines structural blockers between Jackrose's definitions and upstream Asahi installer requirements.

## 1. Seed Image Archive Format
- **Description**: Jackrose packs rootfs as single `.tar.zst` files.
...
## 2. Relative URLs
- **Description**: Jackrose specifies absolute `url` paths. Upstream may expect relative layout offsets from repository base roots.
...
- **Jackrose extensions**: `first_boot_mode` is Jackrose-specific and may not be consumed by the upstream installer.
