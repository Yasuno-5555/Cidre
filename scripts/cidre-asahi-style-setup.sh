#!/bin/bash
set -eu
# cidre-asahi-style-setup.sh
# Asahi ALARM approach: create proper VG, set up boot files, bless.
# Run this script from macOS Terminal to fix Cidre boot picker visibility.
#
# Reference: https://github.com/asahi-alarm/asahi-alarm-installer

echo "=== Cidre Asahi-Style Setup ==="
echo "This follows the Asahi ALARM approach exactly."
echo ""

MACOS_VGID="08E3B56E-946D-4280-9E3C-1931ED8B8B34"
M1N1_SRC="/Users/yasuno/Projects/Cidre/libexec/m1n1.macho"
STEP2_SRC="/Users/yasuno/Projects/Cidre/scripts/cidre-app-recovery-step2"

# ── Step 1: Nuke all existing Cidre volumes ─────────────────────────
echo "[1/6] Cleaning up existing Cidre volumes..."
...
echo "[2/6] Creating Cidre Volume Group..."
# Data volume (role D)
diskutil apfs addVolume disk3 APFS Cidre -role D
echo "  Data volume: created"
sleep 2

# System volume WITH role S (required by APFS for -groupWith)
diskutil apfs addVolume disk3 APFS Cidre -role S -groupWith disk3s1
...
# Copy step2.sh
if [ -f "$STEP2_SRC" ]; then
    cp "$STEP2_SRC" "$SYSTEM_MOUNT/cidre-step2.sh"
    chmod +x "$SYSTEM_MOUNT/cidre-step2.sh"
...
echo "Preboot: $PREBOOT_MOUNT"
echo ""
echo "Next step: Run bless to set Cidre as default boot:"
echo "  sudo bless --setBoot --device /dev/disk3s2 --user \$USER --stdinpass"
echo ""
echo "Then restart your Mac. The Mac will boot into Cidre (1TR mode)."
echo "In 1TR mode, step2 will auto-run (bputil + kmutil + reboot)."
echo "After that, Cidre will appear in the boot picker!"
echo "============================================"
