#!/bin/bash
# build-jackrose-seed-image.sh: Main orchestrator for Jackrose target image assembly
set -euo pipefail
...
# Load variables
# shellcheck source=profiles/jackrose-seed.conf
source "$PROFILE_PATH"

# Safety path resolution guards
TARGET_ROOTFS="${JACKROSE_ROOTFS_DIR:-}"
...
if [ "$APPLY" -eq 1 ] && [ -z "${JACKROSE_BASE_ROOTFS:-}" ]; then
  echo "ERROR: JACKROSE_BASE_ROOTFS is required for --apply mode." >&2
  exit 1
fi
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
LOG_DIR="$REPO_ROOT/logs/image-build"
LOG_FILE="$LOG_DIR/jackrose-seed-$(date +%Y%m%d-%H%M).log"
mkdir -p "$LOG_DIR"

echo "=== Jackrose Seed Image Builder ==="
echo "Profile loaded: $JACKROSE_PROFILE_NAME ($JACKROSE_ARCH)"
...
if [ -z "${JACKROSE_BASE_ROOTFS:-}" ]; then
  echo "  warning: JACKROSE_BASE_ROOTFS is empty; rootfs extraction will be skipped in dry-run."
else
  echo "  Base rootfs targets: $JACKROSE_BASE_ROOTFS"
fi
echo ""
...
if [ "$DRY_RUN" -eq 1 ]; then
  echo "  [DRY-RUN] would create workspace: $JACKROSE_WORKDIR"
  echo "  [DRY-RUN] would create output dir: $JACKROSE_OUTPUT_DIR"
else
  echo "Preparing workdir and directories..."
  mkdir -p "$JACKROSE_OUTPUT_DIR"
fi
echo ""
...
echo "[Step 5/10] Installing Jackrose packages..."
INSTALL_SCRIPT="$SCRIPT_DIR/scripts/install-local-packages"
if [ "$DRY_RUN" -eq 1 ]; then
  echo "  [DRY-RUN] would execute: $INSTALL_SCRIPT --rootfs $TARGET_ROOTFS --packages $JACKROSE_LOCAL_PACKAGE_DIR --dry-run"
  "$INSTALL_SCRIPT" --rootfs "$TARGET_ROOTFS" --packages "$JACKROSE_LOCAL_PACKAGE_DIR" --dry-run
else
  "$INSTALL_SCRIPT" --rootfs "$TARGET_ROOTFS" --packages "$JACKROSE_LOCAL_PACKAGE_DIR" --apply >> "$LOG_FILE" 2>&1
fi
echo ""
...
if [ "$DRY_RUN" -eq 1 ]; then
  echo "  [DRY-RUN] would execute: $PACK_SCRIPT --rootfs $TARGET_ROOTFS --output-dir $JACKROSE_OUTPUT_DIR --name $JACKROSE_IMAGE_NAME --dry-run"
  "$PACK_SCRIPT" --rootfs "$TARGET_ROOTFS" --output-dir "$JACKROSE_OUTPUT_DIR" --name "$JACKROSE_IMAGE_NAME" --dry-run
else
  "$PACK_SCRIPT" --rootfs "$TARGET_ROOTFS" --output-dir "$JACKROSE_OUTPUT_DIR" --name "$JACKROSE_IMAGE_NAME" --apply >> "$LOG_FILE" 2>&1
fi
echo ""

echo "=== Jackrose Seed Image Builder Process Successful ==="
if [ "$APPLY" -eq 1 ]; then
