#!/bin/bash
# lib/jackrose/ui.sh — Jackrose shared TUI display helpers
# Source this file: source "$JACKROSE_ROOT/lib/jackrose/ui.sh"
set -euo pipefail
...
jackrose_banner() {
  ...
  echo -e "${C_MAGENTA}               Jackrose Installer                      ${C_RESET}"
  echo -e "${C_CYAN}====================================================${C_RESET}"
...
# If JACKROSE_YES is set, auto-confirms.
jackrose_confirm() {
  local prompt="$1"
  local default="${2:-N}"

  if [ "${JACKROSE_YES:-false}" = true ]; then
...
jackrose_brand_promise() {
  echo ""
  echo -e "${C_BOLD}Jackrose will NOT change:${C_RESET}"
...
jackrose_brand_confirm() {
  jackrose_brand_promise
  if ! jackrose_confirm "Continue?"; then
...
# Usage: jackrose_failure_report "pacman database sync failed" \
...
# Usage: jackrose_menu "Title" "option1" "desc1" "option2" "desc2" ...
...
jackrose_next_steps() {
  ...
  echo "  2. Select the 'Jackrose' graphical session at the greeter."
  echo "  3. On first login, run 'jackrose-welcome' for a guided tour."
  echo ""
  echo -e "${C_BOLD}Maintenance Commands:${C_RESET}"
  echo "  * System diagnostics:   jackrose-doctor --daily"
  echo "  * Config reset:         jackrose-repair --configs"
  echo "  * Audio fix:            jackrose-repair --audio"
  echo "  * Full recovery:        jackrose-recovery status"
  echo ""
}
