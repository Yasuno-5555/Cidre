#!/bin/bash
# lib/jackrose/checks.sh — Jackrose shared system check functions
# Source this file: source "$JACKROSE_ROOT/lib/jackrose/checks.sh"
set -euo pipefail
...
check_sudo_configured() {
  command -v sudo >/dev/null 2>&1 && \
    [ -f /etc/sudoers.d/jackrose-wheel ] && \
    grep -q '^%wheel ALL=(ALL:ALL) ALL$' /etc/sudoers.d/jackrose-wheel 2>/dev/null
}
...
# Usage: jackrose_run_base_checks
# Sets: CHECK_OK, CHECK_WARN, CHECK_FAIL
jackrose_run_base_checks() {
  CHECK_OK=0
  CHECK_WARN=0
  CHECK_FAIL=0
