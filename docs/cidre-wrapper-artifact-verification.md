...
> Phase 28 does not extract the Cidre rootfs artifact.
...
```bash
installer/scripts/cidre-wrapper-fetch-artifact \
  --selection installer/wrapper/selected-image.json \
  --download-dir /tmp/cidre-wrapper-artifact \
...
```bash
installer/scripts/validate-cidre-wrapper-artifact \
