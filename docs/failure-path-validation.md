# Cidre OOBE Failure-Path Validation Checklist

This checklist defines validation tests for verification of Cidre OOBE's behavior under abnormal, missing, or corrupt target conditions.
...
- **What Cidre tried**: Evaluated `command -v nmtui-connect` before launching setup.
- **Next steps**: Output clear instructions: `Please install NetworkManager TUI support or use a Cidre seed image.` and exit 1.
- **Logs**: `/var/log/cidre-oobe.log`

### 1.2 NetworkManager Service is Inactive
- **What happened**: `NetworkManager` is not running.
- **What Cidre tried**: Executed `systemctl enable --now NetworkManager` during package install or OOBE initialization.
- **Next steps**: Output connection warning, verify `systemctl` status, allow user to retry connection test.
- **Logs**: `/var/log/cidre-oobe.log`, `journalctl -u NetworkManager`

### 1.3 Network Connection is Skipped
- **What happened**: User selected option to skip network setup.
- **What Cidre tried**: Recorded `network=skipped` in `/var/lib/cidre/oobe.state`.
...
- **What happened**: User typed a system name (e.g. `root`, `cidre`, `alarm`, `nobody`, `systemd-test`).
- **What Cidre tried**: Matched name against validation blocklist.
...
- **What Cidre tried**: Validated syntax using match expressions.
...
- **What Cidre tried**: `/var/lib/cidre/firstboot.done` was not created.
...
- **Logs**: `/var/log/cidre-oobe.log`, `/var/lib/cidre/oobe.state`

### 4.2 Completion Marker Incorrectly Staged
- **What happened**: `firstboot.done` exists in raw seed image.
- **What Cidre tried**: Ran `validate-rootfs` during build.
- **Next steps**: Validator fails built image with error. User must run `rm /var/lib/cidre/firstboot.done` inside target rootfs before building image.
- **Logs**: Build output
